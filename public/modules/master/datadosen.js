var subapp = {
    ...
    form: {
        ...
        clearform: function() {
            ...
            $('#fmutama input, #fmutama textarea, #fmutama select').val('').trigger('change');
            $('#formpage .filediv').html('');
        },
        fillform: function(id, dt) {
            //console.log(dt);
            subapp.data.buffer = dt;
            $('input#nip').val(dt.nip);
            $('input#nbm').val(dt.nbm);
            $('input#namadosen').val(dt.namadosen);
            $('input#tempatlahir').val(dt.tempatlahir);
            $('input#tgllahir').val(dt.cd);
            $('input#ttl').val(dt.ttl);
            $('#idprodi').val(dt.prodi).trigger("change");
            $('#idgender').val(dt.gender).trigger("change");            
            $('textarea#alamat').val(dt.alamat);
            $('input#nohp').val(dt.nohp);            
            $('input#email').val(dt.email);
    ...
    dropdown: {
        ...
        listgender: function() {
            app.ajax({
                type: 'post',
                dataType: 'json',
                url: app.get('serviceurl') + '/listgender',
                success: function(resp) {
                    // console.log(resp)
                    var opt='<option value="0"></option>';
                    for (var i = 0;i<resp.length; i++) {
                        var dt=resp[i];
                        opt +='<option value="'+dt.id+'">'+dt.nm_gender+'</option>'
                    }
                    $('#fmutama select#idgender').html(opt);
                    $('#fmutama select#idgender').select2().trigger('change');
                }
            }); 
        },       
    },
    ...
    upload: function() {
        var d = new FormData();
        d.append('filenya', $('#formpage #filedosen')[0].files[0]);
    ...
    rendertable: function() {
        ...
        subapp.table = $("#tabelutama").DataTable({
            ...
            columns: [
                ...
                { data: 'nbm', className: 'text-left' },
                { data: 'namadosen', className: 'text-left' },
                { data: 'nm_gender', className: 'text-left' },
                { data: 'ttl', className: 'text-left' },
                { data: 'alamat', className: 'text-left text-wrap' },
    ...
    data: {
        ...
        form: function() {
            ...
            out.nip = $('#fmutama input#nip').val();
            out.nbm = $('#fmutama input#nbm').val();
            out.namadosen = $('#fmutama input#namadosen').val();
            out.gender = $('#fmutama select#idgender').val();
            out.tempatlahir = $('#fmutama input#tempatlahir').val();
            out.tgllahir = $('#fmutama input#tgllahir').val();
            out.alamat = $('#fmutama textarea#alamat').val();
            out.nohp = $('#fmutama input#nohp').val();         
            out.email = $('#fmutama input#email').val();   
    ...
    save: function() {
        ...
        app.notif.confirm('Konfirmasi', app.confirm.save('data Dosen'), function() {
            app.ajax({
                type: 'post',
                dataType: 'json',
    ...
    init: function() {
        ...
        $('.master').addClass('collapse show active');
        $('.master').prop('aria-expanded',true);
        $('.dtdosen').addClass('active');
        subapp.rendertable();
        $('#fmutama select#idprodi').select2();
        subapp.dropdown.listprodi();

        $('#fmutama select#idgender').select2();
        subapp.dropdown.listgender();        
