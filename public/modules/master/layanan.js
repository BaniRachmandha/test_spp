var subapp = {
    ...
    form: {
        ...
        fillform: function(id, dt) {
            ...
            $('input#jdllayanan').val(dt.jdl_layanan);
            $('textarea#desklayanan').val(dt.desklayanan);
            $('input#datecreate').val(dt.cd);
            $('input#idfile').val(dt.id_files);
            $('input#jnsfile').val(dt.jnsfile);
            $('textarea#ketlayanan').val(dt.ketlayanan);
    ...
    upload: function() {
        var d = new FormData();
        d.append('filenya', $('#formpage #filelayanan')[0].files[0]);
    ...
    rendertable: function() {
        ...
        subapp.table = $("#tabelutama").DataTable({
            ...
            columns: [
                { data: 'id' },
                { data: 'jdl_layanan', className: 'text-left' },
                { data: 'desklayanan', className: 'text-left' },
                { data: 'datecreate', className: 'text-left' },
                { data: 'jnsfile', className: 'text-left' },
                { data: 'ketlayanan', className: 'text-left' },
    ...
    data: {
        ...
        form: function() {
            ...
            out.jdllayanan = $('#fmutama input#jdllayanan').val();
            out.desklayanan = $('#fmutama textarea#desklayanan').val();
            out.datecreate = $('#fmutama input#datecreate').val();
            out.idfile = $('#fmutama input#idfile').val();
            out.jnsfile = $('#fmutama input#jnsfile').val();
            out.ketlayanan = $('#fmutama textarea#ketlayanan').val();
            return out;
        },
        remove: function(id) {
            var out = {};
    ...
    save: function() {
        ...
        app.notif.confirm('Konfirmasi', app.confirm.save('data layanan akademik'), function() {
            app.ajax({
                type: 'post',
                dataType: 'json',
