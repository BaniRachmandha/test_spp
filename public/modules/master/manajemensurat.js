var subapp = {
    ...
    form: {
        ...
        clearform: function() {
            ...
            $('#id_jnssurat').val(null).trigger('change');
        },
        fillform: function(id, dt) {
            console.log(dt);
            subapp.data.buffer = dt;
            $('input#no_surat').val(dt.no_surat);
            $('#id_jnssurat').val(dt.jns_surat).trigger("change");
            $('textarea#perihal').val(dt.perihal);
            $('input#tglsurat').val(dt.tglsurat);            
            $('input#desksurat').val(dt.desksurat);
    ...
    dropdown: {
        listjnssurat: function() {
            app.ajax({
                type: 'post',
                dataType: 'json',
                url: app.get('serviceurl') + '/listjnssurat',
                success: function(resp) {
                    //console.log(resp)
                    var opt='<option value="0"></option>';
                    for (var i = 0;i<resp.length; i++) {
                        var dt=resp[i];
                        opt +='<option value="'+dt.id+'">'+dt.nm_jnssurat+'</option>'
                    }
                    $('#fmutama select#id_jnssurat').html(opt);
                    $('#fmutama select#id_jnssurat').select2().trigger('change');
                }
            });
        },
    },
    upload: function() {
        var d = new FormData();
        d.append('filenya', $('#formpage #filesurat')[0].files[0]);
    ...
    rendertable: function() {
        ...
        subapp.table = $("#tabelutama").DataTable({
            ...
            columns: [
                { data: 'id' },
                { data: 'nm_jnssurat', className: 'text-left' },
                { data: 'no_surat', className: 'text-left' },
                { data: 'perihal', className: 'text-left text-wrap' },
                { data: 'tglsurat', className: 'text-left' },
                { data: 'desksurat', className: 'text-left' },
            ...
            columnDefs: [
                {
                    ...
                    render: function(data, type, row) {
                        return '<div class="hstack gap-2">\
                                        ...
                                        <button class="btn btn-sm btn-soft-primary btdownload" title="Download"><i class="ri-download-fill align-bottom"></i></button>\
                                    </div>';
                    }
                },                                     
            ],
    ...
    data: {
        ...
        form: function() {
            ...
            out.no_surat = $('#fmutama input#no_surat').val();
            out.jns_surat = $('#fmutama #id_jnssurat').find("option:selected").val();
            out.perihal = $('#fmutama textarea#perihal').val();
            out.tglsurat = $('#fmutama input#tglsurat').val();            
            out.desksurat = $('#fmutama input#desksurat').val();            
    ...
    save: function() {
        ...
        app.notif.confirm('Konfirmasi', app.confirm.save('data Manajemen Surat'), function() {
            app.ajax({
                type: 'post',
                dataType: 'json',
    ...
    init: function() {
        ...
        subapp.rendertable();
        $('#fmutama select#id_jnssurat').select2();
        subapp.dropdown.listjnssurat();
